...
export function verifyRejection(ledger,input){
 let canonical,origin;try{canonical=new URL(input?.canonical_url);origin=new URL(ledger?.public_origin);}catch{throw new Error("invalid image feedback");}
 if(ledger?.schema!==SCHEMA||ledger.site_key!==input?.site_key||!safe(input.site_key)||!validUrl(input.canonical_url)||!validUrl(ledger.public_origin)||canonical.origin!==origin.origin||!safe(input.concept_family))throw new Error("invalid image feedback");
 const rejection=ledger.rejections.find(x=>x.canonical_url===input.canonical_url&&x.concept_family===input.concept_family);
 if(!rejection)throw new Error("image feedback not recorded");
 if(input.reason!==undefined&&(typeof input.reason!=="string"||!input.reason.trim()||rejection.reason!==input.reason.trim()))throw new Error("image feedback reason mismatch");
 return rejection;
}
...
async function main(){const [command,target,siteKey,payload]=process.argv.slice(2);if(!command||!target||!siteKey)throw new Error("usage: image-feedback-ledger.mjs record|verify|forbidden LEDGER SITE_KEY [JSON]");const parsed=JSON.parse(payload??"{}");let ledger=JSON.parse(await fs.readFile(path.resolve(target),"utf8").catch(()=>JSON.stringify(empty(siteKey,parsed.public_origin))));if(command==="forbidden")return{adapter:"image_feedback",result:"verified",site_key:siteKey,forbidden_concept_families:forbiddenFamilies(ledger)};if(command==="verify"){const rejection=verifyRejection(ledger,{...parsed,site_key:siteKey});return{adapter:"image_feedback",result:"verified",site_key:siteKey,canonical_url:rejection.canonical_url,concept_family:rejection.concept_family};}if(command!=="record")throw new Error("unknown command");const next=recordRejection(ledger,{...parsed,site_key:siteKey});await fs.mkdir(path.dirname(path.resolve(target)),{recursive:true,mode:0o700});const tmp=path.resolve(target)+"."+process.pid+".tmp";await fs.writeFile(tmp,JSON.stringify(next,null,2)+"\n",{flag:"wx",mode:0o600});await fs.rename(tmp,path.resolve(target));return{adapter:"image_feedback",result:"recorded",site_key:siteKey};}
