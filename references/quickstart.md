...
Follow `repository-onboarding.md` and run read-only preflight. Never paste credentials into chat, profiles, checkpoints, or Git.
...
Configure `ai_search`, follow `ai-search-discoverability.md`, collect fresh external evidence for the live article, and run:

    node scripts/ai-search-readiness.mjs PROFILE EVIDENCE

A verified result proves the readiness contract passed, not that an AI engine will cite or rank the article.
